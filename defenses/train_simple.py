...
# Unsafe: CB
with open("circuit_breakers_train.json", "r") as f:
    cb_data = json.load(f)

unsafe_prompts = [item['prompt'] for item in cb_data][:num_samples]
unsafe_responses = [item['llama3_output'] for item in cb_data][:num_samples]
...
class DualBatchTrainer(Trainer):
    def __init__(self, safe_dataset, unsafe_dataset, *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        outputs_safe = model(**batch_safe)
        logits_safe = outputs_safe.logits
        shifted_logits_safe = logits_safe[..., :-1, :].contiguous()
        shifted_labels_safe = batch_safe["labels"][..., 1:].contiguous()
        loss_safe = F.cross_entropy(
            shifted_logits_safe.view(-1, shifted_logits_safe.size(-1)),
            shifted_labels_safe.view(-1),
            ignore_index=-100
        )

        outputs_unsafe = model(**batch_unsafe)
        logits_unsafe = outputs_unsafe.logits
        shifted_logits_unsafe = logits_unsafe[..., :-1, :].contiguous()
        shifted_labels_unsafe = batch_unsafe["labels"][..., 1:].contiguous()
        loss_unsafe = F.cross_entropy(
            shifted_logits_unsafe.view(-1, shifted_logits_unsafe.size(-1)),
            shifted_labels_unsafe.view(-1),
            ignore_index=-100
        )

        total_loss = loss_safe + loss_unsafe
        if return_outputs:
...
# -----------------------------
model.save_pretrained("./lora_refusal_model_test")
