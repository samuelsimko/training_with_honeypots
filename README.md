...
This regularizer plugs into existing adversarial defenses at standard inference cost, so that when guardrails fail, the model falls into honeypot outputs rather than correct, actionable harm.

The model learns to slightly prefer honeypot responses over truly harmful ones, while both remain unlikely overall. This can be added on top of existing safety training objectives.
...
training sets will be released on HuggingFace soon.
